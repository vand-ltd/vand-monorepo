...
export default function CreateSponsoredArticlePage() {
  const t = useTranslations('createArticle');
  const ts = useTranslations('createSponsoredArticle');
  ...
  const articleMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(ts('articleCreated'));
      router.push('/');
    },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    onError: (error: any) => {
      const message = error?.response?.data?.message || ts('articleFailed');
      toast.error(message);
    },
  });
              ...
              <div className="flex items-center gap-2">
                ...
                {userRole !== 'reporter' && (
                  <button
                    ...
                    onClick={handleSubmit}
                    disabled={articleMutation.isPending}
                    className="flex items-center gap-1.5 px-4 h-9 text-sm font-semibold text-white bg-gradient-to-r from-[#F59E0B] to-[#D97706] rounded-lg hover:opacity-90 transition-all shadow-sm"
                  >
                    {articleMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
                    {t('publish')}
                  </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                  ...
                  <div className="p-8">
                    <div className="flex items-center gap-2 mb-4">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold text-[#D97706] bg-[#F59E0B]/10 rounded-full uppercase tracking-wide">
                        <BadgeDollarSign className="w-3 h-3" />
                        Sponsored
                      </span>
                      {sponsoredBy && (
                        <span className="text-xs text-gray-500 dark:text-gray-400">by {sponsoredBy}</span>
                      )}
                    </div>
                ...
                <>
                  ...
                  >
                    {coverImage ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-12 text-gray-400 dark:text-gray-500 group-hover:text-[#F59E0B] transition-colors">
            ...
            <div className="space-y-6">
              {!isPreview && (
                <>
                  {/* Sponsorship card */}
                  <div className="bg-white dark:bg-gray-800 rounded-xl border border-[#F59E0B]/40 shadow-sm">
                    <div className="px-5 py-4 border-b border-[#F59E0B]/20 rounded-t-xl bg-[#F59E0B]/5">
                    ...
                    <div className="p-5 space-y-4">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          placeholder={ts('sponsoredByPlaceholder')}
                          className="w-full h-10 px-3 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#F59E0B]"
                        />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-700 dark:text-gray-300">
                          {ts('sponsoredUntilLabel')}
                        </label>
                        <input
                          type="date"
                          value={sponsoredUntil}
                          onChange={(e) => setSponsoredUntil(e.target.value)}
                          className="w-full h-10 px-3 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[#F59E0B]"
                        />
                      </div>
                    </div>
                  </div>
              ...
              {/* Tips */}
              <div className="bg-gradient-to-br from-[#F59E0B]/5 to-[#D97706]/5 dark:from-[#F59E0B]/10 dark:to-[#D97706]/10 rounded-xl border border-[#F59E0B]/20 p-5">
