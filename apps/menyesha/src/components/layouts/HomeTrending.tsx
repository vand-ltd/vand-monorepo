...
export async function HomeTrending() {
  ...
  let items = pickList(await fetchJson(`/api/menyesha/articles/trending?language=${locale}&limit=6`));
  if (items.length === 0) {
    items = pickList(await fetchJson(`/api/menyesha/articles?language=${locale}&page=1&limit=6`));
  }
