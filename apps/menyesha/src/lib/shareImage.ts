...
export async function downloadCardPng(node: HTMLElement, filename: string): Promise<void> {
  ...
  // Mobile Safari/Chrome often rasterize the card BEFORE an embedded image has
  // decoded, dropping the cover from the export (desktop is fast enough to dodge
  // it). Force every <img> to finish decoding first...
  await Promise.all(
    Array.from(node.querySelectorAll('img')).map((img) =>
      (img.decode ? img.decode() : Promise.resolve()).catch(() => undefined)
    )
  );
  // ...then render a couple of warm-up passes: html-to-image's first pass on
  // Safari/mobile frequently returns before inlined images load, so the final
  // pass is the reliable one.
