...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (isMatchSlug(match)) {
    ...
    const description = `${home} vs ${away} ${tabName} — ${comp} in Rwanda. Lineups, match events, scores and stats on ${BRAND_NAME}.`;
    return {
      title,
      description,
  ...
  const description = `${comp} ${tab}: live scores, results, fixtures and standings from Rwanda, updated in real time on ${BRAND_NAME}.`;
  return {
    title,
    description,
    keywords: RW_KEYWORDS,
