  ...
  return (
    ...
    >
      ...
      >
        ...
        <SocialIcons size={d.foot * 1.15} color="rgba(255,255,255,0.8)" platforms={GRAPHIC_PLATFORMS} showHandles />
        <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: d.foot }}>{BRAND_DOMAIN}</span>
      </div>
    </div>
  );
});
...
export function MatchShareModal({
  ...
  const download = async () => {
    ...
    setDownloading(true);
    try {
      await downloadCardPng(cardRef.current, `tugezo-${slugify(`${homeName}-${awayName}`).slice(0, 40)}-${size}.png`);
    } finally {
