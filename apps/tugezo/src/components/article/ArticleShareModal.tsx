...
import { BRAND_DOMAIN, LOGO_DARK_PATH, BRAND_NAME } from '@/lib/brand';
    ...
    >
      ...
      <div style={{ position: 'absolute', left: d.pad, right: d.pad, bottom: d.pad }}>
        ...
        <div style={{ marginTop: d.pad * 0.55, display: 'flex', alignItems: 'center', gap: 16 }}>
          {logo ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={logo} alt={BRAND_NAME} style={{ height: d.logo, width: 'auto' }} />
          ) : (
            <span style={{ color: '#fff', fontWeight: 800, fontSize: d.logo * 0.7 }}>{BRAND_NAME}</span>
          )}
          <span style={{ color: 'rgba(255,255,255,0.72)', fontSize: d.foot }}>{BRAND_DOMAIN}</span>
        ...
        <div style={{ marginTop: d.pad * 0.32, borderTop: '1px solid rgba(255,255,255,0.14)', paddingTop: d.pad * 0.3 }}>
          {/* Icons only — the social accounts are still named @menyesha, so
              printing a handle here would contradict the Tugezo wordmark.
              Add `handle` back once the accounts are renamed. */}
          <SocialIcons size={d.foot * 1.15} color="rgba(255,255,255,0.82)" platforms={GRAPHIC_PLATFORMS} showHandles />
        </div>
      </div>
    </div>
  );
...
export function ArticleShareModal({
  ...
  const caption = [
    ...
    excerpt?.trim(),
    url,
    `#${BRAND_NAME} #Rwanda${categoryName ? ` #${categoryName.replace(/[^a-zA-Z0-9]/g, '')}` : ''}`,
  ]
    .filter(Boolean)
    .join('\n\n');
  ...
  const download = async () => {
    ...
    setDownloading(true);
    try {
      await downloadCardPng(cardRef.current, `tugezo-${slugify(title).slice(0, 40)}-${size}.png`);
    } finally {
